 ...
 */
public class MemberGiftMoneyNoticeListener implements ChannelAwareMessageListener {

    private static final Logger logger = Logger.getLogger(MemberGiftMoneyNoticeListener.class);
    ...
    public void onMessage(Message message, Channel channel) throws Exception {
        ...
        if (obj instanceof Map<?, ?>) {
            ...
            String userName = cpaMsg.get("userName").toString();
            String cardNo = cpaMsg.get("cardNo").toString();
            String giftMoney = cpaMsg.get("giftMoney").toString();
            String giveTime = cpaMsg.get("giveTime").toString();
            flag = templateNoticeService.sendMemberGiftMoney(title, remark, storeId, url, openId, storeName,
                    userName, giftMoney, giveTime, cardNo);
        } else {
